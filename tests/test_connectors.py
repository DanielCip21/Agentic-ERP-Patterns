...
class TestDynamics365Connector:
    ...
    def test_get_account_returns_dict(self):
        ...
        respx.get(f"{_D365_BASE}/accounts(ACC-001)").mock(
            return_value=httpx.Response(
                200, json={"accountid": "ACC-001", "name": "Contoso"}
            )
        )
    ...
    def test_create_lead_returns_dict(self):
        ...
        respx.post(f"{_D365_BASE}/leads").mock(
            return_value=httpx.Response(
                201, json={"leadid": "LEAD-001", "lastname": "Lead"}
            )
        )
        result = self._make_connector().create_lead(
    ...
    def test_update_opportunity_returns_dict(self):
        ...
        result = self._make_connector().update_opportunity(
            "OPP-001", {"estimatedvalue": 50000}
        )
    ...
    def test_create_contact_returns_dict(self):
        ...
        respx.post(f"{_D365_BASE}/contacts").mock(
            return_value=httpx.Response(
                201, json={"contactid": "CON-001", "firstname": "Jane"}
            )
        )
        result = self._make_connector().create_contact(
...
class TestSalesforceConnector:
    ...
    def test_instantiation(self):
        assert (
            self._make_connector().config.instance_url
            == "https://testorg.my.salesforce.com"
        )
    ...
    def test_soql_query_returns_dict(self):
        respx.get(f"{_SF_BASE}/query").mock(
            return_value=httpx.Response(
                200,
                json={
                    "totalSize": 1,
        ...
        result = self._make_connector().soql_query(
    ...
    def test_get_record_returns_dict(self):
        respx.get(f"{_SF_BASE}/sobjects/Lead/00Q0000001").mock(
            return_value=httpx.Response(
                200, json={"Id": "00Q0000001", "LastName": "Smith"}
            )
        )
    ...
    def test_create_record_returns_success(self):
        respx.post(f"{_SF_BASE}/sobjects/Account").mock(
            return_value=httpx.Response(
                201, json={"id": "001", "success": True, "errors": []}
            )
        )
        result = self._make_connector().create_record(
    ...
    def test_update_record_returns_no_content(self):
        ...
        result = self._make_connector().update_record(
            "Account", "0010000001", {"Industry": "Technology"}
        )
    ...
    def test_upsert_record_returns_dict(self):
        ...
        result = self._make_connector().upsert_record(
            "Account", "External_Id__c", "EXT-001", {"Name": "Upserted"}
        )
        assert isinstance(result, dict)
...
class TestPowerPlatformConnector:
    ...
    def test_list_flows_returns_list(self):
        respx.post(_AAD_TOKEN_URL).mock(return_value=httpx.Response(200, json=_TOKEN))
        respx.get(f"{_PP_FLOW_BASE}/flows").mock(
            return_value=httpx.Response(
    ...
    def test_trigger_flow_returns_dict(self):
        ...
        result = self._make_connector().trigger_flow(
            "flow-001", {"order_id": "ORD-001"}
        )
    ...
    def test_get_flow_run_status_returns_dict(self):
        ...
        respx.get(f"{_PP_FLOW_BASE}/flows/flow-001/runs/run-001").mock(
            return_value=httpx.Response(
                200, json={"name": "run-001", "status": {"code": "Running"}}
            )
        )
    ...
    def test_list_flow_runs_returns_list(self):
        respx.post(_AAD_TOKEN_URL).mock(return_value=httpx.Response(200, json=_TOKEN))
        respx.get(f"{_PP_FLOW_BASE}/flows/flow-001/runs").mock(
            return_value=httpx.Response(
    ...
    def test_create_dataverse_row_returns_dict(self):
        respx.post(_AAD_TOKEN_URL).mock(return_value=httpx.Response(200, json=_TOKEN))
        respx.post(f"{_PP_DV_BASE}/cr123_orders").mock(
            return_value=httpx.Response(
                201, json={"cr123_ordersid": "row-001", "cr123_name": "Test"}
            )
        )
        result = self._make_connector().create_dataverse_row(
    ...
    def test_update_dataverse_row_returns_no_content(self):
        respx.post(_AAD_TOKEN_URL).mock(return_value=httpx.Response(200, json=_TOKEN))
        respx.patch(f"{_PP_DV_BASE}/cr123_orders(row-001)").mock(
            return_value=httpx.Response(204)
        )
        result = self._make_connector().update_dataverse_row(
            "cr123_orders", "row-001", {"cr123_status": "shipped"}
        )
        assert result.get("status") == "no_content"
    ...
    def test_query_dataverse_returns_list(self):
        respx.post(_AAD_TOKEN_URL).mock(return_value=httpx.Response(200, json=_TOKEN))
        respx.get(f"{_PP_DV_BASE}/cr123_orders").mock(
            return_value=httpx.Response(
    ...
    def test_delete_dataverse_row_returns_no_content(self):
        respx.post(_AAD_TOKEN_URL).mock(return_value=httpx.Response(200, json=_TOKEN))
        respx.delete(f"{_PP_DV_BASE}/cr123_orders(row-001)").mock(
            return_value=httpx.Response(204)
        )
...
class TestAzureAIConnector:
    def _make_connector(self, with_search: bool = False) -> AzureAIConnector:
        extra = (
            {"search_endpoint": _AI_SEARCH_BASE, "search_api_key": "search-key"}
            if with_search
            else {}
        )
    ...
    def test_chat_complete_returns_dict(self):
        ...
        result = self._make_connector().chat_complete(
    ...
    def test_analyze_document_returns_dict(self):
        respx.post(
            f"{_AI_BASE}/documentintelligence/documentModels/prebuilt-invoice:analyze"
        ).mock(return_value=httpx.Response(202, json={"operationId": "op-001"}))
        result = self._make_connector().analyze_document(
    ...
    def test_get_supported_models_returns_dict(self):
        respx.get(f"{_AI_BASE}/documentintelligence/documentModels").mock(
            return_value=httpx.Response(
                200, json={"value": [{"modelId": "prebuilt-invoice"}]}
            )
        )
    ...
    def test_search_index_returns_dict(self):
        respx.post(f"{_AI_SEARCH_BASE}/indexes/knowledge-base/docs/search").mock(
            return_value=httpx.Response(
                200, json={"value": [{"id": "doc-1", "content": "reset guide"}]}
            )
        )
        result = self._make_connector(with_search=True).search_index(
    ...
    def test_vector_search_returns_dict(self):
        ...
        result = self._make_connector(with_search=True).vector_search(
            "kb-index", [0.1, 0.2, 0.3]
        )
        assert isinstance(result, dict) and "value" in result
    ...
    def test_upload_documents_returns_dict(self):
        respx.post(f"{_AI_SEARCH_BASE}/indexes/kb-index/docs/index").mock(
            return_value=httpx.Response(
                200, json={"value": [{"key": "doc-1", "status": True}]}
            )
        )
...
class TestDataverseConnector:
    ...
    def test_instantiation(self):
        assert (
            self._make_connector().config.environment_url
    ...
    def test_query_returns_dict(self):
        ...
        respx.get(f"{_DV_BASE}/accounts").mock(
            return_value=httpx.Response(
    ...
    def test_get_record_returns_dict(self):
        respx.post(_DV_TOKEN_URL).mock(return_value=httpx.Response(200, json=_TOKEN))
        respx.get(f"{_DV_BASE}/accounts(A1)").mock(
            return_value=httpx.Response(
    ...
    def test_create_returns_dict(self):
        respx.post(_DV_TOKEN_URL).mock(return_value=httpx.Response(200, json=_TOKEN))
        respx.post(f"{_DV_BASE}/leads").mock(
            return_value=httpx.Response(
                201, json={"leadid": "L1", "subject": "Test Lead"}
            )
        )
        result = self._make_connector().create(
            "leads", {"subject": "Test Lead", "firstname": "Test"}
        )
        assert isinstance(result, dict) and "leadid" in result
    ...
    def test_update_returns_no_content(self):
        respx.post(_DV_TOKEN_URL).mock(return_value=httpx.Response(200, json=_TOKEN))
        respx.patch(f"{_DV_BASE}/accounts(row-001)").mock(
            return_value=httpx.Response(204)
        )
        result = self._make_connector().update(
            "accounts", "row-001", {"revenue": 1000000}
        )
        assert result.get("status") == "no_content"
    ...
    def test_upsert_returns_no_content(self):
        ...
        result = self._make_connector().upsert(
            "accounts", "cr_externalid", "EXT-001", {"name": "Upserted"}
        )
        assert result.get("status") == "no_content"
    ...
    def test_delete_returns_no_content(self):
        respx.post(_DV_TOKEN_URL).mock(return_value=httpx.Response(200, json=_TOKEN))
        respx.delete(f"{_DV_BASE}/contacts(contact-001)").mock(
            return_value=httpx.Response(204)
        )
    ...
    def test_disassociate_returns_no_content(self):
        respx.post(_DV_TOKEN_URL).mock(return_value=httpx.Response(200, json=_TOKEN))
        respx.delete(
            f"{_DV_BASE}/accounts(acc-001)/account_contacts(contact-001)/$ref"
        ).mock(return_value=httpx.Response(204))
        result = self._make_connector().disassociate(
            "accounts", "acc-001", "account_contacts", "contact-001"
        )
        assert result.get("status") == "no_content"
    ...
    def test_batch_returns_dict(self):
        respx.post(_DV_TOKEN_URL).mock(return_value=httpx.Response(200, json=_TOKEN))
        respx.post(f"{_DV_BASE}/$batch").mock(
            return_value=httpx.Response(

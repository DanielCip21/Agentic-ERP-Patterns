...
class TestProcurementAgent:
    def test_direct_text_response(self):
        client = MagicMock()
        client.messages.create.return_value = _make_text_response("Found 2 vendors in raw_materials category.")
    ...
    def test_dispatch_search_vendors(self):
        ...
        result = agent._dispatch_tool("search_vendors", {"category": "raw_materials", "min_rating": 4.0})
    ...
    def test_dispatch_create_purchase_order(self):
        ...
        result = agent._dispatch_tool(
            ...
            {"vendor_id": "VND-001", "items": [{"sku": "SKU-A", "quantity": 10, "unit_price": 5.0}], "total": 50.0},
        )
        assert isinstance(result, dict)

    def test_dispatch_match_invoice_to_po(self):
        agent = ProcurementAgent(client=MagicMock())
        result = agent._dispatch_tool("match_invoice_to_po", {"invoice_id": "INV-001", "po_id": "PO-1001"})
        assert isinstance(result, dict)
...
class TestFinanceAgent:
    def test_direct_text_response(self):
        client = MagicMock()
        client.messages.create.return_value = _make_text_response("There are 2 overdue invoices totalling $16,800.")
    ...
    def test_dispatch_categorize_expense(self):
        agent = FinanceAgent(client=MagicMock())
        result = agent._dispatch_tool("categorize_expense", {"description": "AWS cloud services", "amount": 1200.0})
        assert "category" in result

    def test_dispatch_schedule_payment(self):
        agent = FinanceAgent(client=MagicMock())
        result = agent._dispatch_tool("schedule_payment", {"invoice_id": "INV-2001", "date": "2026-07-01"})
        assert isinstance(result, dict)
...
class TestHRAgent:
    def test_direct_text_response(self):
        client = MagicMock()
        client.messages.create.return_value = _make_text_response("New employee record created for Jane Doe.")
    ...
    def test_tool_use_then_response(self):
        ...
        tool_resp, text_resp = _make_tool_then_text_response(
            ...
            {"name": "Jane Doe", "department": "Engineering", "start_date": "2026-07-01"},
            "tu_hr_001",
    ...
    def test_dispatch_create_employee_record(self):
        ...
        result = agent._dispatch_tool(
            "create_employee_record",
            {"name": "Test Employee", "department": "Sales", "start_date": "2026-08-01"},
        )
    ...
    def test_dispatch_assign_training(self):
        agent = HRAgent(client=MagicMock())
        result = agent._dispatch_tool("assign_training", {"employee_id": "EMP-001", "role": "engineer"})
        assert isinstance(result, dict)

    def test_dispatch_check_compliance_status(self):
        agent = HRAgent(client=MagicMock())
        result = agent._dispatch_tool("check_compliance_status", {"employee_id": "EMP-001"})
        assert isinstance(result, dict)
...
class TestManufacturingAgent:
    def test_direct_text_response(self):
        client = MagicMock()
        client.messages.create.return_value = _make_text_response("Current production schedule has 3 active work orders.")
    ...
    def test_dispatch_explode_bom(self):
        agent = ManufacturingAgent(client=MagicMock())
        result = agent._dispatch_tool("explode_bom", {"product_id": "PROD-001", "quantity": 50})
        assert isinstance(result, dict)
    ...
    def test_dispatch_check_capacity(self):
        agent = ManufacturingAgent(client=MagicMock())
        result = agent._dispatch_tool("check_capacity", {"workcenter_id": "WC-001", "date": "2026-07-15"})
        assert isinstance(result, dict)

...
def create_employee_record(
    name: str, department: str, start_date: str
) -> dict[str, Any]:
...
def assign_training(employee_id: str, role: str) -> dict[str, Any]:
    ...
    courses = _ROLE_TRAINING_MAP.get(
        role_lower, ["general_onboarding", "security_awareness"]
    )
...
def check_compliance_status(employee_id: str) -> dict[str, Any]:
    ...
    completed = random.sample(
        _COMPLIANCE_CHECKS, k=random.randint(2, len(_COMPLIANCE_CHECKS))
    )
    ...
    return {
        ...
        "compliance_percentage": round(
            len(completed) / len(_COMPLIANCE_CHECKS) * 100, 1
        ),
